...
export default function ProjectDetailsPage() {
  ...
  const exportMatrixToExcel = () => {
    ...
    // Define headers (without Generated By Model and Created At)
    const headers = [
      'Requirement ID',
    ...
    // Build data rows
    const data = matrixEntries.map(entry => {
      const requirement = requirements.find(r => r.id === entry.requirement_id);
      const document = documents.find(d => d.id === entry.document_id);
      return {
        'Requirement ID': requirement?.requirement_id || 'N/A',
        'Requirement Description': requirement?.description || '',
        'Category': requirement?.category || '',
        'Priority': requirement?.priority || '',
        'Document': document?.original_filename || 'N/A',
        'Spec Reference': entry.spec_reference || '',
        'Supplier Response': entry.supplier_response || '',
        'Justification': entry.justification || '',
        'Compliance Status': entry.compliance_status || '',
        'Test Reference': entry.test_reference || '',
        'Risk Assessment': entry.risk_assessment || '',
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Traceability Matrix');

    // Generate Excel file and download
    const fileName = `traceability_matrix_${project?.name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`;
